...
is stop somebody reporting a bug. See `docs/bug-reports.md`.
...
| **Remote-pod (#1) cluster** (`runpod_client.py`, `remote_run.py`, `remote_upscale_engine.py`, `runpod_provision.py`, `ssh_setup.py`; plus `pod/worker.py`, `pod/deadman.py`, `pod/provision.sh`) | Remote upscaling on a rented RunPod GPU (shipped 0.3.1; onboarding in 0.3.2). `runpod_client` = the stdlib control plane (create/start/stop/terminate/inspect pods + network volumes, plus the live GPU and data-center catalogs that drive the pickers). The GUI's live GPU picker (0.3.3) shows only what is deployable right now in the volume's region, cheapest-first, because a curated picklist went stale and offered cards nobody could rent. **On the v1 escape hatch that catalog is GraphQL and so is pod creation** (`deploy_pod` → `podFindAndDeployOnDemand`), because v1's REST create enum 400s on newer cards (Blackwell PRO 4000/4500) the catalog lists with live stock; `CREATABLE_GPU_IDS` documents that limitation. **v2 has neither problem** and both are ordinary REST calls there. **The control plane moved to RunPod's REST API v2 in 0.6.1** (future-features #25, P0-P4), with v1 + GraphQL kept as a config-only escape hatch (`runpod.api_version`, default `v2`) until their shutdown dates: **REST v1 410s on 2026-11-15, GraphQL in early 2027**. Four things about it are load-bearing enough to state here; **the full record, every measurement and the two dated deletions are in `docs/runpod-notes.md`** ("The API v2 migration"), and nothing should be re-derived from the API docs without reading it. **(1) Every response read goes through a normalisation seam** (`pod_state` / `pod_cost` / `pod_data_center` / `pod_gpu` / `pod_ssh` / `pod_volume_id` / `pod_record` / `pod_runtime_sample` / `volume_data_center` / `unwrap_list` / `error_detail`), which accept every transport's shape at once; nothing outside `runpod_client` reads a RunPod field by name, and a test sweeps every module to keep it that way. That is not tidiness: this module's callers treat None as a DECISION rather than as an absence, and all three known renames fail toward **SPENDING MONEY** (a `desiredStatus` reading None makes `_find_existing_pod` miss the app's own pod and deploy a **second billed one**; it makes `runner_common.remote_pod_stopped` return True unconditionally, so auto-resume ends exactly the runs it exists to rescue; a `costPerHr` reading None makes `funds_guard`'s session cap accrue zero and **never trip**). None of that raises and all of it looks like a normal run, which is why the tests use payloads recorded off real pods rather than synthesised from the spec, and are named for the consequence. **(2) The switch direction is the decision**: v2 default, v1 as a MANUAL hatch, never an automatic per-call retry, because installs do not update in lockstep and because retrying a failed pod CREATE on the other transport can leave two billed pods. **(3) The account balance has no v2 successor at all** and is not coming (re-verified against the live spec: 34 paths, one `/v2/account/*` and it is ssh-keys), so the GraphQL island stays until it 410s; `funds_guard` is fail-OPEN, so what 0.6.1 added is the app SAYING when a configured floor is not being enforced (`Funds: Not published` against `Funds: Unknown`). **(4) v2 answers errors as RFC 9457** (`title`/`status`/`detail`/`errors[]`), where `errors[]` names the rejected field. P4 then took what v2 knows that v1 never did, all of it **advisory by construction** (none may turn a working deploy, run or readout into a failed one): `gpu.minCudaVersion` replacing the `KNOWN_CUDA_VERSIONS` enumeration, which had ALREADY gone stale (the catalog offers an RTX 4090 on CUDA **13.2**, past the end of the table, so v1 deploys silently exclude the newest hosts) while keeping the consumer-card-only policy, since CUDA forward-compat is a hardware fact; a `cudaVersions` pre-flight so "in stock but every new-enough machine is full" stops looking identical to "sold out"; per-DC stock so an empty picker can say whether RunPod is out or only this data center is; **ERROR** in `TERMINAL_STATUSES` (a pod that is never coming up used to cost the full four-minute deploy timeout, billed); `pod_logs`, the pod's own log read from the control plane in the failure path, before the terminate that destroys it, and the one diagnostic that does **not** need the tunnel and worker that are exactly what is missing; `pod_runtime_sample`, a second telemetry source for when the tunnel is what broke, deliberately NOT a drop-in equal (percentages with no capacities, marked `via="api"`, and 0 is a reading); and `account_spend`, which answers what a balance never could -- where the money went, including the volume that bills with nothing running. `remote_run.RemoteSession` orchestrates a run (create→push→start worker→arm dead-man's switch→stream→teardown); `remote_upscale_engine.RemoteUpscaleEngine` is a drop-in for `UpscaleEngine` that streams one image per HTTP request over `ssh -L`. `_push_files` uploads `upscale_engine.py` + `orientation.py` + `exif_copy.py` each run (so an engine-side change like #13a's metadata copy needs no re-provision) plus the whole `upscale` config section as `worker_settings.json`. **The pod name is mode-aware** (0.4.3): Image Upscaler + Tag & Rename share `image-toolbox-remote` (both image-side, safe to reuse each other's pod), the Video Upscaler gets its own `video-toolbox-remote` | and `_find_existing_pod` matches on the same per-mode prefix, so starting an image run and a video run at the same time never makes them reuse (and fight over) one pod. `pod/worker.py` is the resident on-pod worker (serves `/upscale`, `/orient`, `/telemetry`, `/health`); its `--mode` is `full` (loads SeedVR2) or `tag` (skips SeedVR2 and serves `/orient` only — remote Tag & Rename, with `remote_run` also starting `ollama serve` and tunnelling 11434). It also **swaps the resident SeedVR2 DiT** when a job names a different one (0.6.3, #26 Part A, `_ensure_video_model`): the worker used to load ONE model at startup, which broke two things once several models became selectable. A multi-model benchmark had to redeploy a pod per model (pod creation + volume mount + cold start, all billed) instead of paying one weight reload; and a mixed-model video queue, which groups by `(engine, gpu)` with the model deliberately NOT in the key, ran every job on whatever was loaded at boot -- a wrong output that reported success, since `process_job` withheld the model from SeedVR2 jobs. It reloads on a MISMATCH only, so an omitted or matching model costs nothing and every older client is unaffected, and it **closes the old engine before building the new one** (a 7B-to-7B swap holding both would need 30.7 GiB of weights). A swap it cannot perform RAISES rather than falling back to the loaded model: for a benchmark that fallback would file one model's numbers under another's key and be believed. This mirrors the LOCAL path, whose `LocalEngineRouter` already cached by `(etype, model)` and evicted on a change; remote was the odd one out. Before loading the engine it **seeds the seedvr2 validation cache** from the DiT+VAE size+mtime (`_seed_validation_cache`, item 11) so a cache miss never triggers a full 16 GB re-hash on the trusted volume (the ~354 s cold-start worst case); copy-to-local NVMe was rejected (a resident worker loads once per pod, so the extra copy isn't amortised). **`ssh_setup.py` (0.3.2)** = zero-config SSH: locates OpenSSH, generates the app's ed25519 key, reads its public half — handed to each pod via the `PUBLIC_KEY` env so SSH needs no key registered on the RunPod website. `runpod_provision.py` is the dev driver + the GUI's `setup-volume` one-shot (create→provision the model volume→auto-terminate). `pod/upscale_one.py` is a minimal single-image on-pod upscale used to validate the remote stack (the original seed of `pod/worker.py`; reuses the same `UpscaleEngine`). All stdlib + the Windows OpenSSH client; the GUI launches these as subprocesses / background threads. |
...
- `seedvr2/` — the SeedVR2 upscaling engine, cloned from
  ...
  itself is not needed). Treat as vendored/third-party. **The pinned version is
  the v2.5 line, at commit `4490bd1` (v2.5.24, 2025-12-24), which is that repo's
  TIP, not a version behind it** (`$SEEDVR2_COMMIT` in `bootstrap.ps1`). Stated
  here because the date reads as "stale" and invites a bump that does not exist:
  upstream stopped there, and the AInVFX fork that fed most of the late v2.5 work
  is stale at the same date, so **the app is in practice the maintainer of its own
  vendored copy**. Everything v2.5 added is already in: the 7B **Sharp** variants,
  **mixed quantisation** (FP8 blocks + FP16 final layer, which is the shipped 16 GB
  tier), **GGUF** including 4-bit, and the v2.5.5 long-video RAM-leak fix. `gguf`
  comes in via `seedvr2/requirements.txt`, so the quantised path costs no separate
  dependency and has shipped since 0.4.6 (`3b-Q8_0.gguf` is a wizard tier). **All
  ten** hash-pinned DiT weights are now selectable (0.6.3, #26 Part A); five of them
  were unreachable purely because no picklist listed them. What decides which tier to
  RECOMMEND is still unmeasured (#26 Part B).
...
- `docs/` — `known-defects.md` (confirmed bugs, with root cause and
  ...
  **present is not working**. **D5 and D6 are the GUI pair and they rhyme**: a button that
  ...
  the only workable shape), `bug-reports.md` (the as-built record for **Report an issue** #24: what the report
  carries, the four redaction rules and the measurement behind each, and the five things
  building it changed. Read it before touching a redaction rule - two of its findings are
  load-bearing beyond this feature: **collecting less is the only protection a later bug
  in a rule cannot undo**, and **an app that generates text about the user's data has a
  disclosure channel no structural rule will find**),
  `notifications.md` (the Discord/Telegram/ntfy backends: setup, the severity contract, what was rejected), `mqtt-integration.md`
