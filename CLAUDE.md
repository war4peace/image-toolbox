...
guardrails learned from live testing: (1) **GraphQL ≠ REST** — the GraphQL catalog
advertises cards (e.g. the Blackwell PRO 4000/4500) that the REST `create_pod`
enum **rejects with HTTP 400**, so `available_gpus` intersects with a
`CREATABLE_GPU_IDS` set and never offers a non-creatable GPU; (2) a **price
ceiling** (`runpod.max_price_per_hour`, default **$0.50/h**, Settings → Remote)
