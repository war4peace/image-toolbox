...
wins, per-video from the frame dimensions). Runs on a **rented RunPod pod** OR on
...
the author `RATES` table). See `docs/future-features.md` #8 and `scripts/bench_share.py`.
...
- `docs/` — `future-features.md` (roadmap: open milestones #3/#4/#5 + the "decided
  against" record; shipped #1/#2/#6/#7 kept only as a numbering legend), `runpod-notes.md` (remote-pod
  upscaling notes), `video-upscaler.md` / `local-video-upscaler.md` (design + as-built
  notes for the Video Upscaler: remote #2 and the local GPU path #7, both
  shipped/experimental), `Benchmarks.csv`
