...
| [`tests/test_ha_samples.py`](../tests/test_ha_samples.py) | The samples themselves: every file parses, every Jinja template compiles, every topic and entity referenced by the automations **and both dashboards** actually exists (in `mqtt-sensors.yaml` or `template-sensors.yaml`), the retained-route guards are present, the derived progress-percent template renders for a video run / an idle app / a malformed value, `automations-ui.yaml` is **pinned equal** to `automations.yaml` (same automations, minus the `id` / `initial_state` the UI owns, so the two shapes cannot drift), and the two "finished" automations **partition** every runner's real `DONE` payload (rendered against all four, exactly one fires). |
