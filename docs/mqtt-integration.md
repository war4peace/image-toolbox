...
| [`samples/home-assistant/`](../samples/home-assistant/) | The user-facing side: sensors, template sensors, two dashboards, `automations.yaml`, and a README that is the setup guide. |
...
| [`tests/test_ha_samples.py`](../tests/test_ha_samples.py) | The samples themselves: the YAML parses, every Jinja template compiles, every topic and entity referenced actually exists, the retained-route guards are present, and the two "finished" automations **partition** every runner's real `DONE` payload (rendered against all four, exactly one fires). |
