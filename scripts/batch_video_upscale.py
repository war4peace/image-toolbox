...
def iter_videos(src_root, skip_roots=None, pruner=None):
    ...
    `skip_roots` prunes whole subtrees by absolute path (in addition to the derived
    folders `pruner` removes by NAME, which always include the `.imgtbx_video` work
    dir). The caller passes the OUTPUT root here: when it lives inside the source
    tree (the default `<source>/__upscaled__`), the walk would otherwise re-read
    finished upscales as if they were fresh source videos. A skip_root outside
    src_root is simply never encountered (harmless).

    `pruner` is an optional caller-supplied `runner_common.DerivedPruner`: pass one
    to read its `.summary()` afterwards (the GUI reports it on the scan line); one is
    created per walk otherwise. Name-pruning covers what the path prune cannot (#16):
    Conciliation's `__Archive__`, and an EARLIER run's output folder when this run
    ...
    for dirpath, dirnames, filenames in os.walk(src_root):
        pruner.prune(dirnames)                           # our own derived folders (#16)
        keep = []
        for d in dirnames:
...
def walk_videos(src_root, skip_roots=None):
    ...
    work area, this app's other derived folders (#16) and any `skip_roots`, e.g. the
    output tree, so a re-run never tries to upscale its own intermediates, its own
    finished outputs, or an original Conciliation moved into `__Archive__`."""
    pruner = runner_common.DerivedPruner()
    out = list(iter_videos(src_root, skip_roots=skip_roots, pruner=pruner))
