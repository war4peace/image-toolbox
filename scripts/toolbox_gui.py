...
# Foundation (paths, version, config.json + gui_settings.json helpers, the funds/
# mqtt/ollama probes) now lives in gui/common.py. Re-imported here so this module
# and the widget/tab/App classes below keep referencing them by bare name; the
# child scripts and any importer of toolbox_gui see the same names as before.
from gui.common import (
    SCRIPT_DIR, APP_ROOT, APP_TITLE, APP_VERSION, CREATE_NO_WINDOW, PROGRESS_RE,
    CFG, GUI_MARKER, _load_config, save_config, get_default_folder, set_default_folder,
    get_install_mode, update_auto_check_enabled, set_update_auto_check,
    update_skipped_version, set_update_skipped_version, _newest_crash_log,
    _issue_url, report_issue, _FUNDS_GREY, funds_color, config_funds_floor,
    fmt_funds, mqtt_config, mqtt_enabled, ollama_installed, ollama_list_models,
    test_discord_webhook, _resolve_python, PYTHON_EXE, _ollama_release_vram,
    load_settings, save_settings, _geometry_on_screen, SETTINGS_PATH, _GEOMETRY_RE,
)

# Now that APP_VERSION is imported, tag crash logs with it (crash_logger may be
# None if its import failed above; the emergency hook still ran).
if crash_logger:
    crash_logger.set_version(APP_VERSION)


# Generic widgets + text/format helpers now live in gui/widgets.py.
from gui.widgets import (
    Tooltip, sanitize, _fmt_eta, ProgressBar, TelemetryRow, _log_hms,
    LogPane, ConsoleBuffer, LogViewer, _ScrollFrame,
    COST100_TIP_BENCH, COST100_TIP_USER, COST100_TIP_RUN,
)


# The before/after comparison windows now live in gui/comparison.py.
...
# ─────────────────────────────────────────────
# ToolTab now lives in gui/tooltab.py; the tool tabs below subclass it.
