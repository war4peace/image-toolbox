...
- **Compare, two ways** (0.6.0): the comparison window's before/after wipe (shared zoom and pan, so both sides always show the same spot), plus a **Lens** tick-box that magnifies the spot under your mouse as original *and* upscaled **side by side**, so you see the difference in one look instead of sliding the divider back and forth. The upscaled half is shown at the file's real pixels ("1:1") and the original is blown up by exactly the upscale factor, so you are comparing what you actually got. **Scroll to zoom the lens** (1x/2x/4x/8x, the panels grow with it), click the picture to pin it in place. **Esc steps back out one level at a time**: it releases a pinned lens, then turns the lens off, then closes the window. Works on the video comparison window too.
- **Browse photos you upscaled earlier** (0.6.0): the preview strip only lasts as long as the run, so **Browse upscaled…** opens a window over your output folder (folders on the left, a thumbnail wall on the right, 200 at a time), and double-clicking any photo opens the same comparison window, wipe and lens included. It finds each original for you (including ones Tag & Rename has since renamed), and a tick-box will match originals you have moved or renamed by reading the files themselves. Answers "was that batch I ran last week any good?" without hunting two files down in Explorer.
...
- **RAW and DNG photos** (0.6.0): `.dng .cr2 .cr3 .nef .arw .orf .rw2 .raf .pef .srw` are accepted and turned into ordinary JPEGs you can look at, send and print. **It renders your RAW files, it does not develop them:** where your camera stored a full-size preview, that preview is what you get, so the picture is your camera's own rendering; where it did not, the sensor data is developed with neutral defaults. There are no exposure or white-balance controls and there will not be. RAW is a high-resolution format, so a RAW is hardly ever *upscaled* (out of 24 real camera files from 2004-2020, not one was small enough to need it); it is always rendered, and upscaled only if it genuinely is small, with the renders counted separately at the end of a run. Results are named `<name>_raw.jpg`, so shooting RAW+JPEG gives you two files rather than one overwriting the other, and the capture date, camera, lens and GPS come across. **Your RAW files are never touched and never replaced:** Tag & Rename skips them, and Conciliation will never archive or delete a RAW original even when a rendered copy exists.
- **GIF pictures** (0.6.3): a still `.gif` is accepted and upscaled like any other picture, and comes back as `<name>_gif.png`. **PNG, not GIF, on purpose:** a GIF holds at most 256 colours, so writing a 4K result back as a GIF would throw away most of what was just computed, while a PNG keeps every colour *and* the see-through parts. The `_gif` in the name means `logo.gif` and `logo.png` in the same folder stay two separate results instead of one overwriting the other. Conciliation understands the new name and will file the upscaled PNG back in place of the original GIF. **An animated GIF is a different job** and is skipped here with a note pointing you at the Video Upscaler tab, which is the tool that can keep the animation smooth (see below).
...
- **Undo last run.** An archive run can be reversed with one button: every original moves back out of `__Archive__` and every processed file returns to the Processed Files folder, leaving your folders exactly as they were. It works even after closing and reopening the app, and it never overwrites anything: a file you have changed since the run, or a name something else has taken, is left alone and reported. A **Delete** run cannot be undone (the files are gone), so the button stays disabled and tells you so, and the log lists exactly which originals were removed.
...
- **Animated GIFs** (0.6.3): drop an animated `.gif` in and it is upscaled here, as a video, arriving as `<name>_gif_<target>.mp4`. It belongs on this tab rather than the Batch Upscaler because a picture-by-picture upscale invents its detail afresh for every frame, which flickers badly once the frames are played back; the video path keeps one look for the whole animation. **The original timing is preserved exactly**, frame by frame, including GIFs whose frames have uneven delays. See-through parts are filled with a background colour (black by default, changeable in **Settings → Video Upscaler**), because video has no transparency. Looping information is lost, and Conciliation will never replace an animated GIF with the video.
...
- **Auto-tuned:** you pick the target and the GPU; batch size, temporal overlap and the remaining SeedVR2 knobs are resolved for the card's actual VRAM, with automatic out-of-memory recovery. **Ten SeedVR2 models** are selectable (0.6.3), per video, from the **Method** picklist: the 7B family for the best detail down to small, quantised versions that fit a modest card, each listed with what it is good for ("7B FP16 (best detail, default)", "3B Q4 (smallest, lowest VRAM)") rather than by weight name. Five of them existed and were downloadable all along but were not offered anywhere. The same models are in Settings, and the **Benchmark GPU** window can now measure **several of them in one run** so you can compare them on your own card under identical conditions (remotely, that reuses a single rented pod rather than renting one per model).
...
- **Steady up shaky old footage** (0.6.0). Point it at a folder, press **Scan folder**, press Start; your originals are never touched. No GPU, nothing rented, no account: it runs on your own machine and takes a little under the length of each video.
- **Each video is steadied whole**, one after another. Steadying works by watching the camera move across the *whole* clip and then smoothing that path, so it runs in two passes (the first watches, the second writes). Cut into pieces, every join would jolt.
- **Stop and come back.** Videos that already have a result are listed as *Stabilised* and skipped, so re-scanning a folder picks up where you left off. One unreadable video is marked *Failed* and the rest of the batch carries on. You can also send one over from the Video Upscaler (right-click → *Stabilize…*).
- **Watch the before and after, with sound.** Double-click a finished video and the original and the steadied copy play side by side, which is the only way to judge steadiness. The still before/after wipe (with the magnifier) is on the right-click menu, for a close look at the edges.
...
- **Video Upscaler** settings: deliverable codec/quality, SeedVR2 model, the background colour used behind a see-through animated GIF, and the advanced tuning knobs (best left on Auto).
...
| `video`    | Video Upscaler deliverable codec/quality, SeedVR tuning knobs, animated-GIF background colour. |
