...
- **First-start wizard:** on the first launch it detects your GPU and recommends
  ...
  (a smaller card can still run a bigger model, just slower). If your card can
  benefit, it also offers an **optional speed-up for local video** (`torch.compile`):
  it explains the download size and the trade-offs, installs the small Triton piece
  for you, and points you to Microsoft's page for the C++ build tools. It's optional
  and affects speed only, never quality. Re-run the wizard any time from
  **Settings → Re-run first-start wizard**.
