...
def test_build_plan_matches_video_by_lineage(db_conn, tmp_path):
    ...
    plan, folders, kept, *_ = cc.build_plan(orig, proc, tr_index=None, conn=db_conn)
...
def test_build_plan_matches_video_by_lineage_after_rename(db_conn, tmp_path):
    ...
    plan, _folders, _k, *_ = cc.build_plan(orig, proc, tr_index=None, conn=db_conn)
...
def test_build_plan_video_without_lineage_is_skipped(db_conn, tmp_path):
    ...
    plan, folders, _k, *_ = cc.build_plan(orig, proc, tr_index=None, conn=db_conn)
    assert plan == []
...
def test_execute_archives_original_and_moves_video_in(db_conn, tmp_path):
    ...
    plan, _f, _k, *_ = cc.build_plan(orig, proc, tr_index=None, conn=db_conn)
